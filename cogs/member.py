...
class Member:
    ...
    async def remind_me(self, ctx, time, *, message=None):
        ...
        if match:
            ...
            return
        reminder_string = ""
        reminder_string += f'{kwargs["days"]:g} days ' if kwargs.get("days") else ""
        reminder_string += f'{kwargs["hours"]:g} hours ' if kwargs.get("hours") else ""
        reminder_string += f'{kwargs["minutes"]:g} minutes ' if kwargs.get("minutes") else ""
        reminder_string += f'{kwargs["seconds"]:g} seconds ' if kwargs.get("seconds") else ""
        ...
        await ctx.send(f"It's been {sleeptime:g} seconds since {ctx.author.mention} made this reminder:\n{message}")
def setup(bot):
